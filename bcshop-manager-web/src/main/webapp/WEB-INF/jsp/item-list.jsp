...
<table class="easyui-datagrid" id="itemList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/tbitem/list',method:'get',pageSize:30,toolbar:toolbar">
    <thead>
        <tr>
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			...
        			// 加载商品描述
        			$.getJSON('/tbitem/querydesc/'+data.id,function(_data){
        				if(_data.status == 200){
        			...
        			//加载商品规格
        			$.getJSON('/tbitem/paramquery/'+data.id,function(_data){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tbitem/delete",params, function(data){
            			if(data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定下架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tbitem/instock",params, function(data){
            			if(data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定上架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tbitem/reshelf",params, function(data){
            			if(data.status == 200){
